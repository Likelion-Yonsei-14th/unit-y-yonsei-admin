...
- 핀 비주얼을 원형 → **사각형**(`rounded-md` + 반투명 fill + border) 로 교체. 기존 3 상태(focused / isMine / default) + !canEnter 오버레이 상태 유지.
...
- v1 의 슬라이더·카드·DateSelector·BoothMapPicker **외부 계약은 변경하지 않는다.** Canvas 내부 구현만 바뀜.
...
| D4 | 오버레이 위치 기준 | 이미지의 **rendered rect**. `ResizeObserver` + `<img>` 의 `getBoundingClientRect()` 측정. |
...
| D8 | 섹션 스왑 | v1 의 2-레이어 opacity 크로스페이드 유지. ResizeObserver 는 현재 섹션 `<img>` 에 붙음 — 섹션 바뀌면 ref 가 새 요소 가리키며 재측정. |
...
v1 의 2-레이어 `setLayers([prev, section])` + 350ms 타이머 로직 그대로. `ResizeObserver` + `currentImgRef` 는 현재 섹션 `<img>` 에 붙으므로, 섹션 바뀌면 ref 가 새 이미지 가리키고 `onLoad` 에서 `measureImage` 호출해 rect 갱신.
...
8. 중앙 focus 상태에서 핀 재클릭 → `/reservations/:boothId` 이동.
9. `pnpm typecheck` 깨끗 확인.

## 이후 작업

- 이 스펙 승인 후 `writing-plans` 스킬로 구현 플랜 생성. 예상 태스크: (a) types + mapper + seed, (b) Canvas 재작성, (c) 수동 QA. 3~4 커밋 수준.
