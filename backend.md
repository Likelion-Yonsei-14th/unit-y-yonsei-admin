...
- 비밀번호 재발급은 서버에서 12자 영숫자 임시값 생성 → bcrypt(또는 동등) 해시 저장 → 평문은 응답으로만 1회 전달. 사용자는 다음 로그인 시 비번 변경 강제 플래그(`must_change_password`) 가 켜진다 (정책 합의 시 컬럼 추가).
- 계정 활성/비활성 토글은 운영 모델 합의에 따라 제거됨 — 계정은 생성 시점부터 항상 활성, 로그인 차단이 필요하면 비밀번호 재설정 또는 삭제로 해결.
...
2. **유저 목록 + 역할 변경 + 비밀번호 재설정** — Super/Master 의 첫 번째 관리 업무
...
- [ ] 비밀번호 정책 (최소 길이, 허용 문자) · `must_change_password` 플래그 컬럼 추가 시점
- [ ] 사용자 본인 비밀번호 변경 화면 (현재는 운영자 발급 임시 비번 전제) — 1차 릴리스 범위 밖
